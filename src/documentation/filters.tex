...
% A4 is standard outside North/South America (slighter taller and thinner than letter)
\usepackage[twoside, a4paper, lmargin=2.5cm, rmargin=2cm, bmargin=2cm, tmargin=2cm]{geometry}
...
% Use UTF8 versions of fonts if available
...
\usepackage[protrusion=true, activate={true,nocompatibility}, final, tracking=true, kerning=true, spacing=false, factor=1100]{microtype}
% Reference management
\usepackage[sorting=nty, backref=true, backend=biber, hyperref=true, sortcites=true]{biblatex}
% Basic Pretty plots
\usepackage{tikz}
% Expanded Pretty plots
\usepackage{pgfplots}
% Figure placement
\usepackage{float}
...
%\usepackage{enumitem}
% Adjust size of tikz/pgfplots figures like you can with includegraphics
%\usepackage{adjustbox}
% Publication quality tables
%\usepackage{booktabs}
% Adds some easy to use commands for writing equations
\usepackage{physics}
% Provides greater functionality for creating custom functions (used by some packages)
\usepackage{xparse}
% Allows to reference things without referring to their type
\usepackage[capitalise]{cleveref}
% Can add a list of terms and acronyms to the document, include it in the toc, automake the glossary
\usepackage[automake, acronym, toc]{glossaries-extra}
% Can be used to add appendicies to the document
%\usepackage{appendix}
% Better control over the number of columns per section
\usepackage{multicol}
% Allows for table elements to span more than one row
%\usepackage{multirow}
%-------------------------------------------------------------------------
% End Packages
%-------------------------------------------------------------------------
...
% Not so ugly anymore!
\hypersetup{colorlinks=true, linkcolor=black, citecolor=blue, filecolor=magenta, urlcolor=blue, pdftitle={Filtering Seismic time series Data}}
\urlstyle{same}
% Pgfplot compatibility
\pgfplotsset{compat=1.18}
% Abbreviation style of acronyms
...
% Acronyms
%-------------------------------------------------------------------------
\newacronym{pssp}{PsSp}{Passive-source Seismic-Processing}
\newacronym{fft}{FFT}{Fast Fourier Transform}
\newacronym{ifft}{IFFT}{Inverse Fast Fourier Transform}
\newglossaryentry{period}{
    name=period,
    description={The amount of time it takes for a process to complete a full cycle.}
}
\newglossaryentry{frequency}{
    name=frequency,
    description={The number of cycles completed in one second.}
}
\newglossaryentry{wavelength}{
    name=wavelength,
    description={The distance over which a wave completes a full cycle.}
}
\newglossaryentry{fourierSeries}{
    name=Fourier series,
    description={An infinite-series representation of a function.}
}
\makeglossaries
%-------------------------------------------------------------------------
% End Acronyms
...
\begin{tcolorbox}[enhanced]
    ...
    with most of the focus placed on the specific filters implemented in \glsxtrshort{pssp}. This is intended to serve as a basic guide on the what and how of filtering seismic data without going through
...
potential meanings. In this case I'm referring to the first deviation from zero amplitude (often referred to as the \textit{first break}).} of approximately
one second\footnote{While it is \textit{technically} true in this case that the signal has infinite duration and therefore the first deviation from zero is \textit{technically} at $-\infty$, real data
always has some background noise that renders this technicality purely \textit{academic/pedantic}.},
...
\noteworthy{f(t) = \cos(8\pi(t-3))e^{-\left(\frac{2\pi(t-3)^{2}}{9}\right)}}{Schematic Time Series}
...
% I dislike how absurdly large of a gap get placed beneath a figure. This also affects its designated location, even if it ended up being placed
% elsewhere due to placement constraints
\begin{center}
    \begin{figure}[ht]
        \begin{tikzpicture}
            \begin{axis}[
...
Test equation reference for \cref{Butterworth Low-pass Filter}.
