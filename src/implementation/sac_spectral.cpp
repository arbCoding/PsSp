...
{
    if (sac.iftype != 1) { std::cerr << "Incorrect iftype: expected `1` (SAC ITIME), received `" << sac.iftype << "`\n"; return; }
    double* signal = (double*) fftw_malloc(sizeof(double) * sac.data1.size());
    for (std::size_t i{0}; i < sac.data1.size(); ++i) { signal[i] = sac.data1[i]; }
...
{
    if (sac.iftype !=2) { std::cerr << "Incorrect iftype: expected `2` (SAC IRLIM), received `" << sac.iftype << "`\n"; return; }
    fftw_complex* spectrum = (fftw_complex*) fftw_malloc(sizeof(fftw_complex) * sac.data1.size());
    for (std::size_t i{0}; i < sac.data1.size(); ++i)
    {
    ...
    fftw_cleanup();
    const double norm{std::sqrt(sac.npts)};
    for (std::size_t i{0}; i < sac.data1.size(); ++i) { sac.data1[i] = signal[i] / norm; }
    fftw_free(signal);
...
{
    if (sac.iftype != 3) { std::cerr << "Incorrect `iftype`: expected `3` (SAC IAMPH), received `" << sac.iftype << "`\n"; return; }
